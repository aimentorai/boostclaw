...
# Post-installation script for BoostClaw on Linux
...
# Create symbolic link for BoostClaw app binary
if [ -x /opt/BoostClaw/BoostClaw ]; then
    ln -sf /opt/BoostClaw/BoostClaw /usr/local/bin/BoostClaw 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/BoostClaw/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/BoostClaw/chrome-sandbox' || true
else
    chmod 0755 '/opt/BoostClaw/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/BoostClaw/resources/apparmor-profile'
    APPARMOR_PROFILE_TARGET='/etc/apparmor.d/BoostClaw'
...
echo "BoostClaw has been installed successfully."
