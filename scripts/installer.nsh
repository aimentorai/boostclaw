; BoostClaw Custom NSIS Installer/Uninstaller Script
;
...
!macro customCheckAppRunning
  ...
  DetailPrint "Extracting BoostClaw runtime files. This can take a few minutes on slower disks or while antivirus scanning is active."
  ...
  ${if} $R0 == 0
    ...
    # This covers BoostClaw.exe (multiple Electron processes), openclaw-gateway.exe,
  ...
  ; Even if BoostClaw.exe was not detected as running, orphan child processes
  ...
  ; PowerShell path-based kill may miss processes if the old BoostClaw was installed
  ...
  ; "Can't modify BoostClaw's files" retry loop -> "BoostClaw 无法关闭" dialog.
  ;
  ...
  ; files (antivirus, indexing) causing a blocking "BoostClaw 无法关闭" dialog.
...
; "BoostClaw 无法关闭" retry dialog when the old uninstaller fails.
;
...
!macro customInstall
  ...
  ; Wait 60s before starting deletion to avoid I/O contention with BoostClaw's
...
!macro customUnInstall
  ...
  _cu_removeData:
    ; Kill any lingering BoostClaw processes (and their child process trees) to
    ...
    ; Check AppData\Local\BoostClaw
    IfFileExists "$LOCALAPPDATA\BoostClaw\*.*" 0 _cu_localDone
      Sleep 3000
      RMDir /r "$LOCALAPPDATA\BoostClaw"
      IfFileExists "$LOCALAPPDATA\BoostClaw\*.*" 0 _cu_localDone
        nsExec::ExecToStack 'cmd.exe /c rd /s /q "$LOCALAPPDATA\BoostClaw"'
        Pop $0
        Pop $1
    _cu_localDone:

    ; Check AppData\Roaming\BoostClaw
    IfFileExists "$APPDATA\BoostClaw\*.*" 0 _cu_roamingDone
      Sleep 3000
      RMDir /r "$APPDATA\BoostClaw"
      IfFileExists "$APPDATA\BoostClaw\*.*" 0 _cu_roamingDone
        nsExec::ExecToStack 'cmd.exe /c rd /s /q "$APPDATA\BoostClaw"'
        Pop $0
        Pop $1
    _cu_roamingDone:
    ...
    StrCpy $R3 ""
    IfFileExists "$LOCALAPPDATA\BoostClaw\*.*" 0 +2
      StrCpy $R3 "$R3$\r$\n  • $LOCALAPPDATA\BoostClaw"
    IfFileExists "$APPDATA\BoostClaw\*.*" 0 +2
      StrCpy $R3 "$R3$\r$\n  • $APPDATA\BoostClaw"
