...
test.describe('BoostClaw chat execution graph', () => {
  test('does not render the execution graph while keeping tool actions visible', async ({ launchElectronApp }) => {
    ...
    try {
      ...
      await expect(
        page.locator('[data-testid="chat-tool-card"]').getByText('sessions_spawn', { exact: true }),
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="chat-tool-card"]').getByText('sessions_yield', { exact: true }),
      ).toBeVisible();
