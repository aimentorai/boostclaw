...
- 在 CoPaw所在虚拟环境中安装 Ollama：`pip install 'boostclaw[ollama]'`。
...
> 如果在过程中遇到 `Ollama SDK not installed. Install with: pip install 'boostclaw[ollama]'`的提示，请先确认是否已经在 ollama.com 下载 Ollama，并在 CoPaw所在虚拟环境中执行过 `pip install 'boostclaw[ollama]'`。如果想删除某个模型，点击 Ollama 卡片右下角的 **模型**，在模型列表中，点击想要删除的模型右侧的 **垃圾桶按钮**，二次确认后即可删除。
>
> **Docker 用户：** 如果 CoPaw 运行在 Docker 容器中，`localhost` 指向的是容器自身而非宿主机。请将 Ollama 的 Base URL 改为 `http://host.docker.internal:11434`（并在 `docker run` 命令中添加 `--add-host=host.docker.internal:host-gateway`）。详见 [README 的 Docker 章节](https://github.com/aimentorai/boostclaw#使用-docker)。
>
...
>
> **Docker 用户：** 如果 CoPaw 运行在 Docker 容器中，`localhost` 指向的是容器自身而非宿主机。请将 LM Studio 的 Base URL 改为 `http://host.docker.internal:1234/v1`（并在 `docker run` 命令中添加 `--add-host=host.docker.internal:host-gateway`）。详见 [README 的 Docker 章节](https://github.com/aimentorai/boostclaw#使用-docker)。
