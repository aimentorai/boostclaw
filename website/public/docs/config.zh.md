...
| 变量                     | 默认值             | 说明                                                                                                                                            |
| ------------------------ | ------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| `COPAW_WORKING_DIR`      | `~/.boostclaw`     | 工作目录；config、心跳、jobs、chats、skills、memory 都在这下面                                                                                  |
| `COPAW_SECRET_DIR`       | `~/.boostclaw.secret` | 敏感数据目录（工作目录的同级目录）；存放 `providers.json`（模型配置、API Key）和 `envs.json`（环境变量）。Docker 中默认为 `/app/working.secret` |
...
- 默认一切都在 **`~/.boostclaw`**；想改就设 **`COPAW_WORKING_DIR`** 等环境变量。
