...
<p align="center">
  <img src="src/assets/logo.svg" width="128" height="128" alt="BoostClaw Logo" />
</p>

<h1 align="center">BoostClaw</h1>
...
<p align="center">
  ...
  <a href="#why-BoostClaw">Why BoostClaw</a> •
...
<p align="center">
  ...
  <img src="https://img.shields.io/github/downloads/ValueCell-ai/BoostClaw/total?color=%23027DEB" alt="Downloads" />
...
**BoostClaw** bridges the gap between powerful AI agents and everyday users. Built on top of [OpenClaw](https://github.com/OpenClaw), it transforms command-line AI orchestration into an accessible, beautiful desktop experience—no terminal required.

Whether you're automating workflows, managing AI-powered channels, or scheduling intelligent tasks, BoostClaw provides the interface you need to harness AI agents effectively.

BoostClaw comes pre-configured with best-practice model providers and natively supports Windows as well as multi-language settings.
...
Building AI agents shouldn't require mastering the command line. BoostClaw was designed with a simple philosophy: **powerful technology deserves an interface that respects your time.**

| Challenge | BoostClaw Solution |
|-----------|----------------|
...
BoostClaw is built directly upon the official **OpenClaw** core. Instead of requiring a separate installation, we embed the runtime within the application to provide a seamless "battery-included" experience.
...
When you target another agent with `@agent`, BoostClaw switches into that agent's own conversation context directly instead of relaying through the default agent. Agent workspaces stay separate by default, and stronger isolation depends on OpenClaw sandbox settings.
...
BoostClaw now also bundles Tencent's official personal WeChat channel plugin, so you can link WeChat directly from the Channels page with an in-app QR flow.
...
BoostClaw also pre-bundles full document-processing skills (`pdf`, `xlsx`, `docx`, `pptx`), deploys them automatically to the managed skills directory (default `~/.boostclaw/openclaw/skills`) on startup, and enables them by default on first install. Additional bundled skills (`find-skills`, `self-improving-agent`, `tavily-search`, `brave-web-search`) are also enabled by default; if required API keys are missing, OpenClaw will surface configuration errors in runtime.  
...
### 🌙 Adaptive Theming
Light mode, dark mode, or system-synchronized themes. BoostClaw adapts to your preferences automatically.

### 🚀 Startup Launch Control
In **Settings → General**, you can enable **Launch at system startup** so BoostClaw starts automatically after login.
...
Download the latest release for your platform from the [Releases](https://github.com/ValueCell-ai/BoostClaw/releases) page.

#### Build from Source

```bash
# Clone the repository
git clone https://github.com/ValueCell-ai/BoostClaw.git
cd BoostClaw
...
> Note for Moonshot (Kimi): BoostClaw keeps Kimi web search enabled by default.  
> When Moonshot is configured, BoostClaw also syncs Kimi web search to the China endpoint (`https://api.moonshot.cn/v1`) in OpenClaw config.

### Proxy Settings

BoostClaw includes built-in proxy settings for environments where Electron, the OpenClaw Gateway, or channels such as Telegram need to reach the internet through a local proxy client.
...
- If advanced proxy fields are left empty, BoostClaw falls back to `Proxy Server`.
- Saving proxy settings reapplies Electron networking immediately and restarts the Gateway automatically.
- BoostClaw also syncs the proxy to OpenClaw's Telegram channel config when Telegram is enabled.
- Gateway restarts preserve an existing Telegram channel proxy if BoostClaw proxy is currently disabled.
...
BoostClaw can enforce an app-level OIDC login flow in desktop mode:
...
- `BoostClaw_APP_AUTH_ENABLED=1`
- `BoostClaw_APP_AUTH_AUTHORIZATION_ENDPOINT` (default `https://open.am.microdata-inc.com/usercenter/oauth/authorize`)
- `BoostClaw_APP_AUTH_TOKEN_ENDPOINT` (default `https://open.am.microdata-inc.com/usercenter/oauth/token`)
- `BoostClaw_APP_AUTH_CODE_EXCHANGE_ENDPOINT` (optional backend endpoint for exchanging `code/codeVerifier`; avoids storing client secrets in the desktop app)
- `BoostClaw_APP_AUTH_CLIENT_ID` (default `4edb20b8-9bb1-4fe1-9b20-b89bb19fe13a`)
- `BoostClaw_APP_AUTH_TOKEN_AUTH_METHOD` (`auto` / `client_secret_post` / `client_secret_basic` / `none`; default `auto`)
- `BoostClaw_APP_AUTH_CLIENT_SECRET` (required for `client_secret_post` or `client_secret_basic`)
- `BoostClaw_APP_AUTH_REDIRECT_URI` (default `https://open.microdata-inc.com`)
...
BoostClaw employs a **dual-process architecture** with a unified host API layer. The renderer talks to a single client abstraction, while Electron Main owns protocol selection and process lifecycle:

```┌─────────────────────────────────────────────────────────────────┐
│                        BoostClaw Desktop App                         │
│                                                                  │
│  ┌────────────────────────────────────────────────────────────┐  │
...
- BoostClaw is an Electron app, so **one app instance normally appears as multiple OS processes** (main/renderer/zygote/utility). This is expected.
...
- Clicking the window close button (`X`) hides BoostClaw to tray; it does **not** fully quit the app. Use tray menu **Quit BoostClaw** for complete shutdown.
...
- uses a temporary BoostClaw `userData` directory
...
We welcome contributions from the community! Whether it's bug fixes, new features, documentation improvements, or translations—every contribution helps make BoostClaw better.
...
BoostClaw is built on the shoulders of excellent open-source projects:
...
### BoostClaw Partner Program 🚀

We're launching the BoostClaw Partner Program and looking for partners who can help introduce BoostClaw to more clients, especially those with custom AI agent or automation needs.

Partners help connect us with potential users and projects, while the BoostClaw team provides full technical support, customization, and integration.
...
<p align="center">
  <img src="https://api.star-history.com/svg?repos=ValueCell-ai/BoostClaw&type=Date" alt="Star History Chart" />
</p>
...
BoostClaw is released under the [MIT License](LICENSE). You're free to use, modify, and distribute this software.
