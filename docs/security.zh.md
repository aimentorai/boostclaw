...
boostclaw 内置了安全功能，保护你的 Agent 免受恶意输入和不安全技能的影响。这些功能在控制台 **设置 → 安全** 中配置，也可以通过 `config.json` 进行设置。
...
在控制台（**设置 → 安全 → 技能扫描器 → 扫描模式**）或通过环境变量 `BOOSTCLAW_SKILL_SCAN_MODE`（`block`、`warn` 或 `off`）设置。环境变量优先于配置文件。
