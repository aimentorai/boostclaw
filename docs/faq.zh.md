...
boostclaw 支持三种安装方式，详情请见文档 [快速开始](https://boostclaw.com/docs/quickstart)：
...
2. pip 安装（需 Python >= 3.12, < 3.14）

```bash
pip install boostclaw
```

3. 桌面应用（Beta）：无需命令行，下载即用，详见 [桌面应用指南](https://boostclaw.com/docs/desktop)。
...
3. 如果你使用的是桌面应用，请前往 [Releases](https://github.com/aimentorai/boostclaw/releases) 下载最新安装包/压缩包，并直接覆盖安装现有版本。

如果你使用的是 CLI / 服务端安装方式，升级后请重新执行 `boostclaw app` 启动服务。
...
控制台默认地址为 `http://127.0.0.1:8088/`，使用默认配置快速初始化后，可以进入控制台快捷自定义相关内容。详情请见[快速开始](https://boostclaw.com/docs/quickstart)。

### 如何配置模型

在控制台进入 **设置 → 模型** 中进行配置，详情请见文档 [模型](https://boostclaw.com/docs/models)：

- 云端模型：填写提供商 API Key（如 ModelScope、DashScope 或自定义提供商），再选择活跃模型。

命令行也可使用 `boostclaw models` 系列命令完成配置、下载和切换，详情请见文档 [CLI → 模型与环境变量 → boostclaw models](https://boostclaw.com/docs/cli#boostclaw-models)。
...
最方便的定时任务创建方式是，在你想要获取定时任务返回结果的频道，与boostclaw对话，让boostclaw帮你创建一个定时任务。例如，可以直接与boostclaw对话：“帮我创建一个定时任务，每隔五分钟提醒我喝水。”之后可以在控制台中看到状态为已启用的定时任务。

如果定时任务没有正常启动，可以按照以下几个步骤排查：

1. 首先确认 boostclaw 服务是在正常运行中的。
...
6. 排查结束后，如果想确认一下定时任务是否创建成功，且能成功触发，可以点击 **立即执行**，若成功创建，则可在对应频道收到回复。或者也可以直接与 boostclaw 对话：“帮我触发一下刚刚创建的提醒喝水定时任务”。

   ![exec](https://img.alicdn.com/imgextra/i4/O1CN017tycJh1ZPhO5XMuAu_!!6000000003187-2-tps-3020-762.png)

### 如何管理Skill

进入控制台 **智能体 → 技能**，可以启用/禁用技能、创建自定义技能、以及从 Skills Hub 中导入技能。详情请见文档 [Skills](https://boostclaw.com/docs/skills)。

### 如何配置MCP

进入控制台 **智能体 → MCP**，进行 MCP 客户端的启用/禁用/删除/创建，详情请见文档 [MCP](https://boostclaw.com/docs/mcp)。
...
boostclaw 支持百炼 Coding Plan 获取的 API key。如果仍报错，请重点检查：
...
为了加快修复与排查，共建良好社区生态，建议遇到报错时，首选在 boostclaw 的 GitHub 仓库中提 [issue](https://github.com/aimentorai/boostclaw/issues)，请附上完整报错信息，并上传错误详情文件。
...
请将后面的`/var/folders/.../copaw_query_error_qzbx1mv1.json`文件一并上传，同时提供你当前的模型提供商、模型名和 boostclaw 的具体版本。
