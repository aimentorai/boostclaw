...
ClawX では、アプリ全体の OIDC ログインゲートを有効化できます。
...
- `CLAWX_APP_AUTH_AUTHORIZATION_ENDPOINT`（既定 `https://open.am.microdata-inc.com/usercenter/oauth/authorize`）
- `CLAWX_APP_AUTH_TOKEN_ENDPOINT`（既定 `https://open.am.microdata-inc.com/usercenter/oauth/token`）
- `CLAWX_APP_AUTH_CODE_EXCHANGE_ENDPOINT`（任意のバックエンド交換エンドポイント。`code/codeVerifier` を交換し、desktop app に client secret を置かないために使用）
- `CLAWX_APP_AUTH_CLIENT_ID`（既定 `4edb20b8-9bb1-4fe1-9b20-b89bb19fe13a`）
- `CLAWX_APP_AUTH_TOKEN_AUTH_METHOD`（`auto` / `client_secret_post` / `client_secret_basic` / `none`。既定は `auto`）
- `CLAWX_APP_AUTH_CLIENT_SECRET`（`client_secret_post` または `client_secret_basic` で必須）
- `CLAWX_APP_AUTH_REDIRECT_URI`（既定 `https://open.microdata-inc.com`）
- `CLAWX_APP_AUTH_COOKIE_NAME`（既定 `Auth-Graviteeio-APIM`。リダイレクトページの cookie から読み取ります）
- `CLAWX_APP_AUTH_APP_CALLBACK_URI`（既定 `boostclaw://auth/callback`）
- `CLAWX_APP_AUTH_SCOPE`（既定 `openid profile`）
