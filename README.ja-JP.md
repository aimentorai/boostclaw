...
<p align="center">
  <a href="#機能">機能</a> •
  <a href="#なぜBoostClawなのか">なぜBoostClawなのか</a> •
  <a href="#はじめに">はじめに</a> •
...
**BoostClaw**は、強力なAIエージェントと日常のユーザーとの間のギャップを埋めます。[OpenClaw](https://github.com/OpenClaw)をベースに構築されており、コマンドラインによるAIオーケストレーションを、アクセスしやすく美しいデスクトップ体験に変換します。ターミナルは不要です。

ワークフローの自動化、AI搭載チャネルの管理、インテリジェントなタスクのスケジューリングなど、BoostClawはAIエージェントを効果的に活用するために必要なインターフェースを提供します。

BoostClawはベストプラクティスのモデルプロバイダーが事前設定されており、Windowsおよび多言語設定をネイティブにサポートしています。
...
AIエージェントの構築にコマンドラインの習得は不要であるべきです。BoostClawはシンプルな哲学のもとに設計されました：**強力な技術には、あなたの時間を尊重するインターフェースがふさわしい。**

| 課題 | BoostClawのソリューション |
|------|----------------------|
...
BoostClawは公式の**OpenClaw**コアを直接ベースに構築されています。別途インストールを必要とせず、アプリケーション内にランタイムを組み込むことで、シームレスな「バッテリー同梱」体験を提供します。
...
`@agent` で別のエージェントを選ぶと、BoostClaw はデフォルトエージェントを経由せず、そのエージェント自身の会話コンテキストへ直接切り替えます。各エージェントのワークスペースは既定で分離されていますが、より強い実行時分離は OpenClaw の sandbox 設定に依存します。
...
BoostClaw には Tencent 公式の個人 WeChat チャンネルプラグインも同梱されており、Channels ページからアプリ内 QR フローで直接 WeChat を連携できます。
...
BoostClaw はドキュメント処理スキル（`pdf`、`xlsx`、`docx`、`pptx`）もフル内容で同梱し、起動時に管理スキルディレクトリ（既定 `~/.boostclaw/openclaw/skills`）へ自動配備し、初回インストール時に既定で有効化します。追加の同梱スキル（`find-skills`、`self-improving-agent`、`tavily-search`、`brave-web-search`）も既定で有効化されますが、必要な API キーが未設定の場合は OpenClaw が実行時に設定エラーを表示します。  
...
### 🌙 アダプティブテーマ
ライトモード、ダークモード、またはシステム同期テーマ。BoostClawはあなたの好みに自動的に適応します。

### 🚀 自動起動設定
**設定 → 通用** から **システム起動時に自動起動** を有効化すると、ログイン後に BoostClaw が自動的に起動します。
...
[Releases](https://github.com/ValueCell-ai/BoostClaw/releases)ページから、お使いのプラットフォーム向けの最新リリースをダウンロードしてください。
...
BoostClawを初めて起動すると、**セットアップウィザード**が以下の手順をガイドします：

1. **言語と地域** – 使用する言語・地域の設定
2. **AIプロバイダー** – APIキーまたは OAuth で任意に追加できます。スキップして後から設定することもできます
...
BoostClawには、Electron、OpenClaw Gateway、またはTelegramなどのチャネルがローカルプロキシクライアントを介してインターネットにアクセスする必要がある環境向けに、組み込みのプロキシ設定が含まれています。
...
- 高度なプロキシフィールドが空の場合、BoostClawは`プロキシサーバー`にフォールバックします。
- プロキシ設定を保存すると、Electronのネットワーク設定が即座に再適用され、ゲートウェイが自動的に再起動されます。
- BoostClawはTelegramが有効な場合、プロキシをOpenClawのTelegramチャネル設定にも同期します。
- BoostClawのプロキシが無効な状態では、Gatewayの通常再起動時に既存のTelegramチャネルプロキシ設定を保持します。
...
### 任意のデスクトップログイン（システムブラウザ + PKCE）

BoostClaw では、アプリ全体の OIDC ログインゲートを有効化できます。

- メインプロセスがログインを開始し、Electron 内蔵ログインウィンドウを開く
- OAuth のリダイレクト URI は既定で `https://open.microdata-inc.com`
- Web 側リダイレクト後、`code/state` をアプリコールバック URI（既定 `BoostClaw://auth/callback`）へ再転送する必要があります
- Authorization Code + PKCE（`S256`）でトークン交換を実行

環境変数:

- `BoostClaw_APP_AUTH_ENABLED=1`
- `BoostClaw_APP_AUTH_AUTHORIZATION_ENDPOINT`（既定 `https://open.am.microdata-inc.com/usercenter/oauth/authorize`）
- `BoostClaw_APP_AUTH_TOKEN_ENDPOINT`（既定 `https://open.am.microdata-inc.com/usercenter/oauth/token`）
- `BoostClaw_APP_AUTH_CODE_EXCHANGE_ENDPOINT`（任意のバックエンド交換エンドポイント。`code/codeVerifier` を交換し、desktop app に client secret を置かないために使用）
- `BoostClaw_APP_AUTH_CLIENT_ID`（既定 `4edb20b8-9bb1-4fe1-9b20-b89bb19fe13a`）
- `BoostClaw_APP_AUTH_TOKEN_AUTH_METHOD`（`auto` / `client_secret_post` / `client_secret_basic` / `none`。既定は `auto`）
- `BoostClaw_APP_AUTH_CLIENT_SECRET`（`client_secret_post` または `client_secret_basic` で必須）
- `BoostClaw_APP_AUTH_REDIRECT_URI`（既定 `https://open.microdata-inc.com`）
- `BoostClaw_APP_AUTH_COOKIE_NAME`（既定 `Auth-Graviteeio-APIM`。リダイレクトページの cookie から読み取ります）
- `BoostClaw_APP_AUTH_APP_CALLBACK_URI`（既定 `BoostClaw://auth/callback`）
- `BoostClaw_APP_AUTH_SCOPE`（既定 `openid profile`）
- `BoostClaw_APP_AUTH_PROMPT`（既定は空で、再ログインを強制しません。毎回再認証したい場合は `login` を指定）

---

## アーキテクチャ

BoostClawは、**デュアルプロセス + Host API 統一アクセス**構成を採用しています。Renderer は単一クライアント抽象を呼び出し、プロトコル選択とライフサイクルは Main が管理します：
...
- BoostClaw は Electron アプリのため、**1つのアプリインスタンスでも複数プロセス（main/renderer/zygote/utility）が表示される**のが正常です。
...
- ウィンドウの閉じるボタン（`X`）は既定でトレイへ最小化する動作で、完全終了ではありません。完全終了する場合はトレイメニューの **Quit BoostClaw** を使用してください。
...
コミュニティからのコントリビューションを歓迎します！バグ修正、新機能、ドキュメントの改善、翻訳など、あらゆる貢献がBoostClawをより良くするのに役立ちます。
...
BoostClawは優れたオープンソースプロジェクトの上に構築されています：
...
### BoostClaw パートナープログラム 🚀

BoostClaw パートナープログラムを開始します。特に、カスタム AI エージェントや自動化ニーズを持つより多くの顧客に BoostClaw を紹介してくださるパートナーを募集しています。

パートナーの皆さまには、見込みユーザーや案件との接点づくりを担っていただき、BoostClaw チームは技術サポート、カスタマイズ、統合を全面的に提供します。
...
<p align="center">
  <img src="https://api.star-history.com/svg?repos=ValueCell-ai/BoostClaw&type=Date" alt="スター履歴チャート" />
</p>
...
BoostClawは[MITライセンス](LICENSE)の下でリリースされています。本ソフトウェアの使用、変更、配布は自由に行えます。
