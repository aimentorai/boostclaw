...
- `CLAWX_APP_AUTH_AUTHORIZATION_ENDPOINT`（默认 `https://open.am.microdata-inc.com/usercenter/oauth/authorize`）
- `CLAWX_APP_AUTH_TOKEN_ENDPOINT`（默认 `https://open.am.microdata-inc.com/usercenter/oauth/token`）
- `CLAWX_APP_AUTH_CODE_EXCHANGE_ENDPOINT`（可选后端换 token 地址，用于交换 `code/codeVerifier`，避免在桌面端存放 client secret）
- `CLAWX_APP_AUTH_CLIENT_ID`（默认 `4edb20b8-9bb1-4fe1-9b20-b89bb19fe13a`）
- `CLAWX_APP_AUTH_TOKEN_AUTH_METHOD`（`auto` / `client_secret_post` / `client_secret_basic` / `none`；默认 `auto`）
- `CLAWX_APP_AUTH_CLIENT_SECRET`（`client_secret_post` 或 `client_secret_basic` 时必填）
- `CLAWX_APP_AUTH_REDIRECT_URI`（默认 `https://open.microdata-inc.com`）
- `CLAWX_APP_AUTH_COOKIE_NAME`（默认 `Auth-Graviteeio-APIM`，从重定向页 cookie 中读取）
- `CLAWX_APP_AUTH_APP_CALLBACK_URI`（默认 `boostclaw://auth/callback`）
- `CLAWX_APP_AUTH_SCOPE`（默认 `openid profile`）
