...
<p align="center">
  <a href="#功能特性">功能特性</a> •
  <a href="#为什么选择-BoostClaw">为什么选择 BoostClaw</a> •
  <a href="#快速上手">快速上手</a> •
  <a href="#系统架构">系统架构</a> •
  <a href="#开发指南">开发指南</a> •
  <a href="#参与贡献">参与贡献</a>
...
**BoostClaw** 是连接强大 AI 智能体与普通用户之间的桥梁。基于 [OpenClaw](https://github.com/OpenClaw) 构建，它将命令行式的 AI 编排转变为易用、美观的桌面体验——无需使用终端。

无论是自动化工作流、连接通讯软件，还是调度智能定时任务，BoostClaw 都能提供高效易用的图形界面，帮助你充分发挥 AI 智能体的能力。

BoostClaw 预置了最佳实践的模型供应商配置，原生支持 Windows 平台以及多语言设置。
...
构建 AI 智能体不应该需要精通命令行。BoostClaw 的设计理念很简单：**强大的技术值得拥有一个尊重用户时间的界面。**
...
BoostClaw 直接基于官方 **OpenClaw** 核心构建。无需单独安装，我们将运行时嵌入应用内部，提供开箱即用的无缝体验。
...
当你使用 `@agent` 选择其他智能体时，BoostClaw 会直接切换到该智能体自己的对话上下文，而不是经过默认智能体转发。各 Agent 工作区默认彼此分离，但更强的运行时隔离仍取决于 OpenClaw 的 sandbox 配置。
...
BoostClaw 现在还内置了腾讯官方个人微信渠道插件，可直接在 Channels 页面通过内置二维码流程完成微信连接。
...
BoostClaw 还会内置预装完整的文档处理技能（`pdf`、`xlsx`、`docx`、`pptx`），在启动时自动部署到托管技能目录（默认 `~/.boostclaw/openclaw/skills`），并在首次安装时默认启用。额外预装技能（`find-skills`、`self-improving-agent`、`tavily-search`、`brave-web-search`）也会默认启用；若缺少必需的 API Key，OpenClaw 会在运行时给出配置错误提示。  
...
### 🔐 安全的供应商集成
连接多个 AI 供应商，凭证安全存储在系统原生密钥链中。内置供应商目录现在聚焦国内服务商，如 DeepSeek、千问、Moonshot、火山方舟、SiliconFlow 和 MiniMax 国内站。
如果你通过 **自定义（Custom）Provider** 对接 OpenAI-compatible 网关，可以在 **设置 → AI Providers → 编辑 Provider** 中配置自定义 `User-Agent`，以提高兼容性。
如果兼容网关的 `/models` 因非鉴权原因不可用，BoostClaw 会在校验 API Key 时自动降级为轻量的 `/chat/completions` 或 `/responses` 探测。

### 🌙 自适应主题
支持浅色模式、深色模式或跟随系统主题。BoostClaw 自动适应你的偏好设置。

### 🚀 开机启动控制
在 **设置 → 通用** 中，你可以开启 **开机自动启动**，让 BoostClaw 在系统登录后自动启动。
...
首次启动 BoostClaw 时，**设置向导** 将引导你完成以下步骤：

1. **语言与区域** – 配置你的首选语言和地区
2. **AI 供应商** – 可通过 API 密钥或 OAuth 添加账号，也可以先跳过，稍后再配置
3. **技能包** – 选择适用于常见场景的预配置技能
4. **验证** – 在进入主界面前测试你的配置

如果系统语言在支持列表中，向导会默认选中该语言；否则回退到英文。

> Moonshot（Kimi）说明：BoostClaw 默认保持开启 Kimi 的 web search。  
> 当配置 Moonshot 后，BoostClaw 也会将 OpenClaw 配置中的 Kimi web search 同步到中国区端点（`https://api.moonshot.cn/v1`）。

### 代理设置

BoostClaw 内置了代理设置，适用于需要通过本地代理客户端访问外网的场景，包括 Electron 本身、OpenClaw Gateway，以及 Telegram 这类频道的联网请求。
...
- 如果启用了 Telegram，BoostClaw 还会把代理同步到 OpenClaw 的 Telegram 频道配置中。
- 当 BoostClaw 代理处于关闭状态时，Gateway 的常规重启会保留已有的 Telegram 频道代理配置。
...
BoostClaw 支持启用应用级 OIDC 登录门禁：

- 由主进程发起登录并打开 Electron 内置登录窗口
- OAuth 重定向地址默认使用 `https://open.microdata-inc.com`
- 网页完成重定向后，需要将 `code/state` 二次跳转到应用回调地址（默认 `BoostClaw://auth/callback`）
- 使用 Authorization Code + PKCE（`S256`）完成令牌交换

环境变量：

- `BoostClaw_APP_AUTH_ENABLED=1`
- `BoostClaw_APP_AUTH_AUTHORIZATION_ENDPOINT`（默认 `https://open.am.microdata-inc.com/usercenter/oauth/authorize`）
- `BoostClaw_APP_AUTH_TOKEN_ENDPOINT`（默认 `https://open.am.microdata-inc.com/usercenter/oauth/token`）
- `BoostClaw_APP_AUTH_CODE_EXCHANGE_ENDPOINT`（可选后端换 token 地址，用于交换 `code/codeVerifier`，避免在桌面端存放 client secret）
- `BoostClaw_APP_AUTH_CLIENT_ID`（默认 `4edb20b8-9bb1-4fe1-9b20-b89bb19fe13a`）
- `BoostClaw_APP_AUTH_TOKEN_AUTH_METHOD`（`auto` / `client_secret_post` / `client_secret_basic` / `none`；默认 `auto`）
- `BoostClaw_APP_AUTH_CLIENT_SECRET`（`client_secret_post` 或 `client_secret_basic` 时必填）
- `BoostClaw_APP_AUTH_REDIRECT_URI`（默认 `https://open.microdata-inc.com`）
...
BoostClaw 采用 **双进程 + Host API 统一接入架构**。渲染进程只调用统一客户端抽象，协议选择与进程生命周期由 Electron 主进程统一管理：
...
- BoostClaw 基于 Electron，**单个应用实例出现多个系统进程是正常现象**（main/renderer/zygote/utility）。
...
- 点击窗口关闭按钮（`X`）默认只是最小化到托盘，并不会完全退出应用。请在托盘菜单中选择 **Quit BoostClaw** 执行完整退出。
...
我们欢迎社区的各种贡献！无论是修复 Bug、开发新功能、改进文档还是翻译——每一份贡献都让 BoostClaw 变得更好。
...
我们正在启动 BoostClaw 合作伙伴计划，寻找能够帮助我们将 BoostClaw 介绍给更多客户的合作伙伴，尤其是那些有定制化 AI 智能体或自动化需求的客户。

合作伙伴负责帮助我们连接潜在用户和项目，BoostClaw 团队则提供完整的技术支持、定制开发与集成服务。
...
<p align="center">
  <img src="https://api.star-history.com/svg?repos=ValueCell-ai/BoostClaw&type=Date" alt="Stars 历史图表" />
</p>
...
BoostClaw 基于 [MIT 许可证](LICENSE) 发布。你可以自由地使用、修改和分发本软件。
