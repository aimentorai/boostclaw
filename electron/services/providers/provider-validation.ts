...
function logValidationStatus(provider: string, status: number): void {
  console.log(`[BoostClaw-validate] ${provider} HTTP ${status}`);
}
...
function logValidationRequest(
  ...
  console.log(
    `[BoostClaw-validate] ${provider} request ${method} ${sanitizeValidationUrl(url)} headers=${JSON.stringify(sanitizeHeaders(headers))}`,
  );
}
...
async function validateOpenAiCompatibleKey(
  ...
  if (shouldFallbackFromModelsProbe(modelsResult)) {
    console.log(
      `[BoostClaw-validate] ${providerType} /models returned ${modelsResult.status}, falling back to ${apiProtocol} probe`,
    );
    if (apiProtocol === 'openai-responses') {
...
async function validateAnthropicHeaderKey(
  ...
  if (
    ...
    console.log(
      `[BoostClaw-validate] ${providerType} /models returned error, falling back to /messages probe`,
    );
