...
export class ClawHubService {
  ...
  constructor() {
    // Use the user's OpenClaw config directory (~/.boostclaw/openclaw) for skill management
    ...
    return new Promise((resolve, reject) => {
      ...
      const child = spawn(spawnCmd, spawnArgs, {
        ...
        env: {
          ...
          OPENCLAW_STATE_DIR: this.workDir,
        },
        windowsHide: true,
      });
  ...
  async listInstalled(): Promise<ClawHubInstalledSkillResult[]> {
        ...
        for (const line of lines) {
          ...
          if (match) {
            ...
            results.push({
              ...
              baseDir: path.join(this.workDir, 'skills', match[1]),
            });
          }
        }
      }
    ...
    // Source 2: Skills on disk not tracked by CLI (e.g. preinstalled)
    try {
      const skillsDir = path.join(this.workDir, 'skills');
      if (fs.existsSync(skillsDir)) {
        const entries = fs.readdirSync(skillsDir, { withFileTypes: true });
        for (const entry of entries) {
          if (!entry.isDirectory() || seenSlugs.has(entry.name)) continue;
      ...
      console.error('ClawHub disk scan error:', error);
    }
