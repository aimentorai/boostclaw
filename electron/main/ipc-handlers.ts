      ...
      switch (request.module) {
        ...
        case 'cron': {
          ...
          if (request.action === 'create') {
            ...
            const unsupportedDeliveryError = getUnsupportedCronDeliveryError(
              gatewayInput.delivery.channel
            );
          ...
          if (request.action === 'update') {
            ...
            const deliveryPatch =
              patch.delivery && typeof patch.delivery === 'object'
                ? (patch.delivery as Record<string, unknown>)
                : undefined;
            const deliveryChannel =
              typeof deliveryPatch?.channel === 'string' && deliveryPatch.channel.trim()
                ? deliveryPatch.channel.trim()
                : undefined;
            const deliveryMode =
              typeof deliveryPatch?.mode === 'string' && deliveryPatch.mode.trim()
...
function normalizeCronDelivery(
  ...
  const mode =
    typeof delivery.mode === 'string' && delivery.mode.trim() ? delivery.mode.trim() : fallbackMode;
  const channel =
...
function normalizeCronDeliveryPatch(rawDelivery: unknown): Record<string, unknown> {
  ...
  if ('mode' in delivery) {
    patch.mode =
      typeof delivery.mode === 'string' && delivery.mode.trim() ? delivery.mode.trim() : 'none';
  }
...
function transformCronJob(job: GatewayCronJob) {
  ...
  const channelType = gatewayDelivery.channel
    ? toUiChannelType(gatewayDelivery.channel)
    : undefined;
...
function registerCronHandlers(gatewayManager: GatewayManager): void {
  ...
  // UI-created tasks have no delivery target — results go to the BoostClaw chat page.
    ...
    async (
      ...
      try {
        const gatewayInput = {
          ...
          wakeMode: 'next-heartbeat',
          sessionTarget: 'isolated',
          // UI-created jobs deliver results via BoostClaw WebSocket chat events,
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        console.log(`[BoostClaw-validate] validating provider type: ${providerType}`);
