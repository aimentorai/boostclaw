...
const PREINSTALLED_MARKER_NAME = '.BoostClaw-preinstalled.json';
...
/**
 * Ensure proboost-mcp is configured as an MCP server in openclaw.json.
 *
 * proboost-mcp is an external MCP server accessed via SSE transport.
 * OpenClaw natively supports MCP servers — no plugin wrapper needed.
 * Idempotent: skips if already configured.
 */
export async function ensureProboostMcpConfigured(): Promise<void> {
  return withConfigLock(async () => {
    const config = await readConfig();
    ...
    if (servers['proboost-mcp']) {
      return;
    }

    servers['proboost-mcp'] = {
      url: 'http://mcp.microdata-inc.com/mcp-servers/proboost-mcp/sse',
    };

    await writeConfig(config);
    logger.info('Configured proboost-mcp MCP server in openclaw.json');
  });
}
