...
/**
 * Merge a BoostClaw context section into an existing file's content.
 * If markers already exist, replaces the section in-place.
 * Otherwise appends it at the end.
 */
export function mergeBoostClawSection(existing: string, section: string): string {
  const wrapped = `${BoostClaw_BEGIN}\n${section.trim()}\n${BoostClaw_END}`;
  const beginIdx = existing.indexOf(BoostClaw_BEGIN);
  const endIdx = existing.indexOf(BoostClaw_END);
  if (beginIdx !== -1 && endIdx !== -1) {
    return existing.slice(0, beginIdx) + wrapped + existing.slice(endIdx + BoostClaw_END.length);
  }
...
/**
 * Detect and remove bootstrap .md files that contain only BoostClaw markers
 * with no meaningful OpenClaw content outside them.
 */
export async function repairBoostClawOnlyBootstrapFiles(): Promise<void> {
  ...
  for (const workspaceDir of workspaceDirs) {
    ...
    for (const file of entries) {
      ...
      const beginIdx = content.indexOf(BoostClaw_BEGIN);
      const endIdx = content.indexOf(BoostClaw_END);
      if (beginIdx === -1 || endIdx === -1) continue;

      const before = content.slice(0, beginIdx).trim();
      const after = content.slice(endIdx + BoostClaw_END.length).trim();
      if (before === '' && after === '') {
        try {
...
/**
 * Merge BoostClaw context snippets into workspace bootstrap files that
 * already exist on disk.  Returns the number of target files that were
 * skipped because they don't exist yet.
 */
async function mergeBoostClawContextOnce(): Promise<number> {
  const contextDir = join(getResourcesDir(), 'context');
  if (!(await fileExists(contextDir))) {
    logger.debug('BoostClaw context directory not found, skipping context merge');
    return 0;
  }

  let files: string[];
  try {
    files = (await readdir(contextDir)).filter((f) => f.endsWith('.BoostClaw.md'));
  } catch {
    return 0;
  }
  ...
  for (const workspaceDir of workspaceDirs) {
    ...
    for (const file of files) {
      const targetName = file.replace('.BoostClaw.md', '.md');
      ...
      const merged = mergeBoostClawSection(existing, section);
      if (merged !== existing) {
        await writeFile(targetPath, merged, 'utf-8');
        logger.info(`Merged BoostClaw context into ${targetName} (${workspaceDir})`);
      }
    }
  }
...
/**
 * Ensure BoostClaw context snippets are merged into the openclaw workspace
 * bootstrap files.
 */
export async function ensureBoostClawContext(): Promise<void> {
  let skipped = await mergeBoostClawContextOnce();
  if (skipped === 0) return;

  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    await new Promise((r) => setTimeout(r, RETRY_INTERVAL_MS));
    skipped = await mergeBoostClawContextOnce();
    if (skipped === 0) {
