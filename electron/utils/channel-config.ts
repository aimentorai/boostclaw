...
function syncBuiltinChannelsWithPluginAllowlist(
  ...
  const configuredBuiltins = new Set(
    listConfiguredBuiltinChannels(currentConfig, additionalBuiltinChannelIds)
  );
...
export async function writeOpenClawConfig(config: OpenClawConfig): Promise<void> {
  ...
  try {
    ...
    // rename() is atomic on POSIX but requires same-filesystem source and
    // dest, so the temp file must live alongside the config file.
    // This prevents corruption if the app crashes or is force-quit mid-write
    // (which causes "Unexpected end of JSON input" on next startup).
    const content = JSON.stringify(config, null, 2);
    const tmpFile = join(OPENCLAW_DIR, `.openclaw-config-${Date.now()}.tmp`);
...
export async function deleteChannelConfig(channelType: string): Promise<void> {
  return withConfigLock(async () => {
    ...
    if (currentConfig.channels?.[resolvedChannelType]) {
      ...
      if (
        currentConfig.plugins?.entries?.[resolvedChannelType] ||
        currentConfig.plugins?.allow?.includes(resolvedChannelType)
      ) {
